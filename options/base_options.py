...
class BaseOptions:
    ...
    def initialize(self):
        ...
        self.parser.add_argument(
            ...
            type=int,
            default=1.0,
            help='A prior orientation vector to set the target orientation'
        )
        self.parser.add_argument(
            '--grasps_folder_name',
            type=str,
        ...
        self.parser.add_argument(
            '--orientation_constrained', '-oc',
            action='store_true',
            help='if true, train with orientation constrained'
        )
        self.parser.add_argument(
            '--vanilla', '-vani',
            action='store_true',
            help='if true, train the vanilla model without any constraints'
        )
        self.parser.add_argument(
            '--equivariant', '-eq',
            action='store_true',
            help='if true, train the pitch yaw equivariant network'
        )
        self.parser.add_argument(
            '--debug', '-debug',
            action='store_true',
            help='if true, train with only one camera pose'
        )
        self.parser.add_argument(
            '--clusters', '-cl',
            action='store_true',
            help='if true, train the constrained grasp sampler with multiple query points'
        )
        self.parser.add_argument(
            '--pre_rendered_point_clouds', '-prpc',
            action='store_true',
            help='if true, then we use a dataset with prerendered point-clouds'
        )
        self.parser.add_argument(
        ...
        if args.arch == "evaluator":
            self.parser.add_argument('--evaluator_type',
                                     choices={"pointnet++", "DGCNN"},
                                     default='pointnet++')
            args, _ = self.parser.parse_known_args()
            if args.evaluator_type == "DGCNN":
                self.parser.add_argument('--num_neighbors',
                                         default=32,
                                         type=int)
                self.parser.add_argument('-rif', '--rotation_invariant_features',
                                         action='store_true')
    ...
    def parse(self):
        ...
        assert sum(
            self.opt.dataset_split_ratio) == 1, "The split ratio of the dataset must sum to one."
        ...
        # if not self.opt.pre_rendered_point_clouds and self.opt.constrained and not self.opt.clusters:
        if not self.opt.pre_rendered_point_clouds and not self.opt.clusters:
            self.opt.npoints -= 1
        args = vars(self.opt)
        ...
        if self.opt.is_train:
            ...
            if self.opt.arch == "evaluator":
                name += self.opt.evaluator_type
            if self.opt.orientation_constrained:
                name += "_orientation_constrained"
            if self.opt.vanilla:
                name += "_vanilla"
            if self.opt.equivariant:
                name += "_equivariant"
            if self.opt.debug:
                name += "_debug"
            if self.opt.clusters:
                name += "_clusters"
            if self.opt.pre_rendered_point_clouds:
                name += "_prerendered"
    ...
    def load_opt(self):
        yaml_file = open(self.opt.model_folder+"opt.yaml")

        swap_dataset_path = False
        dataset_root_folder = ''
        if self.opt.dataset_root_folder != '':
            dataset_root_folder = self.opt.dataset_root_folder
            swap_dataset_path = True
        self.opt = EasyDict(yaml.load(yaml_file, Loader=yaml.FullLoader))
        self.opt.is_train = False
        if swap_dataset_path:
            self.opt.dataset_root_folder = dataset_root_folder
