...
class Writer:
    ...
    def print_acc(self, epoch):
        ...
        if self.opt.arch == "evaluator":
            message = 'epoch: {}, TEST ACC: [{:.5} %]\n' \
                .format(epoch, self.accuracy * 100)
        else:
    ...
    def plot_acc(self, epoch):
        if self.display:
            if self.opt.arch == "evaluator":
                self.display.add_scalar('data/test_acc/grasp_prediction', self.accuracy,
                                        epoch)
            else:
        ...
        if self.display:
            if len(point_clouds) > 0:
                try:
                    ...
                    self.display.add_mesh(train_or_test+'/grasps',
                                          vertices=random_point_cloud[0], colors=random_point_cloud[1], global_step=epoch)
                except IndexError:
                    pass
